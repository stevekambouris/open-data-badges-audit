################################################################################
# stage1_15_coding_cases_followup_step05.R
#
# This script uses the results of step 4 of the reviewing/finalising process to
# compare coders' responses for files_exist for each cleaned resolved_url
# found.
...
# Import the finalised step 4 data for the articles.
...
# Get all the unique study_id values from Step 4 of the finalised dataset.
study_ids_to_check <- all_final_coding_step4 %>% 
  distinct(study_id)

# Get all cases in the cleaned coding data which meet these criteria:
# 1) The coder reports a value for resolved_url;
...
# official way to know if a coding record has a valid resolved_url or not.
coding_to_check <- all_clean_coding %>% 
  inner_join(study_ids_to_check, by = "study_id") %>% 
  filter(is.na(resolved_url) == FALSE) %>% 
...
################################################################################
# Section 3: Check the values of files_exist
#
# ************************************* NOTE ***********************************
# This section uses the following functions from the R script
# stage1_11_coding_cases_followup_step03.R:
# - check_disagreement()
# - check_noother()
#
# Make sure these functions have been loaded in the environment before
# proceeding.
# ******************************************************************************
...
fe_disagree <- check_disagreement(in_data = coding_to_check,
                                  checkvar = "files_exist") %>% 
  relocate(link_url_cleaned, .after = link_url_for_cleaning) %>% 
  mutate(use_code = NA_character_) %>% 
  mutate(review_note = NA_character_)

fe_noother <- check_noother(in_data = coding_to_check,
                            checkvar = "files_exist") %>% 
  relocate(link_url_cleaned, .after = link_url_for_cleaning) %>% 
...
write_csv(x = fe_disagree,
          path = here("data", "output", "stage1", "follow_up",
                      "step05_follow_up_disagree_files_exist_iter01.csv"))
write_rds(x = fe_disagree,
          path = here("data", "output", "stage1", "follow_up",
                      "step05_follow_up_disagree_files_exist_iter01.rds"))

# Note the file names here - update with each iteration of the data updates.
write_csv(x = fe_noother,
          path = here("data", "output", "stage1", "follow_up",
                      "step05_follow_up_noother_files_exist_iter01.csv"))
write_rds(x = fe_noother,
          path = here("data", "output", "stage1", "follow_up",
                      "step05_follow_up_noother_files_exist_iter01.rds"))
...
fe_disagree_iter01_results <- read_csv(file = here("data", "raw",
                                                   "stage1_review",
                                                   "stage1_step5_files_exist_disagree - iter01.csv"))

fe_noother_iter01_results <- read_csv(file = here("data", "raw",
                                                  "stage1_review",
